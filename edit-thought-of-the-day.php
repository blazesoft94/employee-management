...
if(isset($_GET['eid'])) {
	...
	$select_l = mysql_query("SELECT * FROM 	thoughts WHERE tid='".$eid."'");

if(isset($_POST['edit_thought'])) {
	$t_title = addslashes($_POST['t_title']);
	$t_details = addslashes($_POST['t_details']);
	$display_date = date('Y-m-d', strtotime($_POST['display_date']));
	$select = mysql_query("SELECT * FROM thoughts WHERE tid='".$eid."'");
	$select11 = mysql_query("SELECT * FROM thoughts WHERE display_date='".$display_date."'");
	$modify_date = date('Y-m-d');
	
	if(empty($t_title) || empty($t_details) || empty($_POST['display_date'])) {
		...
		/*if(mysql_num_rows($select11) == 0) {*/
			if(!file_exists($_FILES['image']['tmp_name']) || !is_uploaded_file($_FILES['image']['tmp_name'])) {
				...
				$insert_details = mysql_query("UPDATE thoughts SET `t_title`='".$t_title."',`t_details`='".$t_details."',`display_date`='".$display_date."',`modify_date`='".$modify_date."' WHERE tid='".$eid."'");
				if($insert_details) {
								// success added
								$msg = '<div class="alert alert-success">
							<a aria-hidden="true" href="#" data-dismiss="alert" class="close">×</a>
							<i class="fa fa-envelope"></i>&nbsp; Thoughts added successfully
						</div>';
							} else {
								// error added
								$msg = '<div class="alert alert-warning">
				...
				if ($_SERVER['REQUEST_METHOD'] == 'POST' AND isset($_FILES['image'])) {
					if( $_FILES['image']['size'] < $max_file_size ){
						...
						if (in_array($ext, $valid_exts)) {
							...
							$insert_details = mysql_query("UPDATE thoughts SET `t_title`='".$t_title."',`t_details`='".$t_details."',`t_img`='".$path."',`display_date`='".$display_date."',`modify_date`='".$modify_date."' WHERE tid='".$eid."'");
							if($insert_details) {
								// success added
								$msg = '<div class="alert alert-success">
							<a aria-hidden="true" href="#" data-dismiss="alert" class="close">×</a>
		...
		/*} else {
			$msg = '<div class="alert alert-warning">
							<a aria-hidden="true" href="#" data-dismiss="alert" class="close">×</a>
							<i class="fa fa-envelope"></i>&nbsp;Display Date already in this list
						</div>';
		}*/
...
<head>
  <title><?php echo $settings['title']; ?> - Edit Thought of the day</title>
  ...
  <div class="content">
    ...
    <div class="content-container">
      ...
      <div class="content-header">
        <h2 class="content-header-title">Edit Thought of the day</h2>
        <ol class="breadcrumb">
        ...
        <div class="col-md-12">
            ...
            <div class="portlet">
                <div class="portlet-header">
	              <h3><i class="fa fa-tasks"></i>    Edit Thoughts date</h3>
	            </div>
...
<?php
$select_list = mysql_query("SELECT * FROM 	thoughts WHERE tid='".$eid."'");
...
$row = mysql_fetch_array($select_list);?>           
            ...
            <div class="col-sm-8">
              <div class="form-group">
                   <label for="t_title">Thoughts Title: <em class="red-col"> *</em></label>
                 	<input type="text" name="t_title" id="t_title" value="<?php echo $row['t_title']; ?>" class="form-control" data-required="true">
                </div>
              <div class="form-group">
            ...
            <div class="col-sm-4">
                ...
                <div class="fileupload fileupload-new" data-provides="fileupload">
                  <div class="fileupload-new thumbnail" style="width: 200px; height: 150px;"><img src="<?php echo $row['t_img']; ?>" alt="Placeholder" /></div>
            ...
            <div class="col-sm-12">
             <div class="form-group">
                  <label for="textarea-input">Thoughts Details: <em class="red-col"> *</em></label>
                  <textarea class="form-control parsley-validated"  rows="2" cols="10" id="t_details" name="t_details" data-minlength="5" data-required="true"><?php echo $row['t_details']; ?></textarea>
                  </div>
                </div>
             
                 <div class="col-sm-6">
                 <input class="btn  btn-secondary"  type="submit" name="edit_thought" id="edit_thought" value="Submit"/>
                </div>
