        ...
        <table 
                ...
                class="table table-bordered table-hover">
                  ...
                  <tbody>
                    ...
                    <?php // total records only showing on
					 if($users['department'] == "BPO") { ?>   <td><div class="form-group"><input type="text" value="<?php echo $info['total_records']; ?>" name="total_records" class="form-control" placeholder="Total Records" /></div></td> <?php } ?>
                ...
                $i += '01';} ?>
                      ...
                      <td><div class="form-group"><textarea id="comments" name="comments" class="form-control" rows="3" data-required="true"><?php echo $info['comments']; ?></textarea></div></td>
...
<?php } }  else { ?>
              ...
              <table 
                ...
                class="table table-bordered table-hover">
                 ...
                 <?php
				 $ee = explode(":",$time_remain);
				  $j = '00';

				while($j < '24') {
					
					// REMAINING TIME AUTOMATIC PRINT
					if($ee[0] == $j) {
						$hrs = "selected";
					} else { $hrs='';
					}
...
<?php if($time_remain == "00:00") { ?>
  ...
  if(isset($_POST['submit']))	{
		...
		while($row = mysql_fetch_array($select_temp)) {
			...
			$success_change = mysql_query("INSERT INTO work_report (`username`,`emp_id`,`project_name`,`type_of_work`,`time_of_work`,`comments`,`total_time`,`report_date`,`total_records`) VALUES ('".$row['username']."','".$row['emp_id']."','".$row['project_name']."','".$row['type_of_work']."','".$row['time_of_work']."','".$row['comments']."','".$fetch['working_hours']."','".$row['report_date']."','".$row['total_records']."')");
